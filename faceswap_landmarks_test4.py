...
# oryginal_image_path = "output/AE/laura_oliwka/img_4.jpg"
# predicted_image_path = "output/AE/laura_oliwka/predicted_img_4.jpg"
...
height, width, _ = predict_image.shape
...
destination_image_grayscale = cv2.cvtColor(predict_image, cv2.COLOR_BGR2GRAY)
...
for destination_face in destination_faces:
    ...
    convexhull_predict = cv2.convexHull(landmarks)
    ...
    background_predict = cv2.bitwise_and(predict_image, predict_image, mask=image_predict_face_mask)
    cv2.imshow('Predict image crop face', background_predict)

    image_predict_face = cv2.bitwise_and(predict_image, predict_image, mask=mask_predict)
...
#             cv2.imshow('Oryginal image mask', image_oryginal_face_mask)
#
#             background_oryginal = cv2.bitwise_and(image, image, mask=image_predict_face_mask)
#             cv2.imshow('Oryginal image crop face', background_oryginal)
#
#             image_oryginal_face = cv2.bitwise_and(image, image, mask=mask_predict)
#             cv2.imshow('Oryginal image face', image_oryginal_face)
#
#
#             image_result = cv2.add(background_oryginal, image_predict_face)
