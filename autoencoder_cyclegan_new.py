...
def get_resnet_generator(
    ...
    x = ReflectionPadding2D(padding=3)(img_input)
...
def get_discriminator(filters=64, kernel_initializer=kernel_init, name=None):
    ...
    img_input = layers.Input(shape=IMAGE_SHAPE, name=name + "_img_input")
    x = layers.Conv2D(filters, 4, strides=1, padding="same", kernel_initializer=kernel_initializer)(img_input)
    ...
    for num_downsample_block in range(3):
        ...
        if num_downsample_block < 2:
            x = downsample(x, filters=num_filters, activation=layers.LeakyReLU(0.2), kernel_size=4, strides=2)
        else:
            x = downsample(x, filters=num_filters, activation=layers.LeakyReLU(0.2), kernel_size=4, strides=1)

    x = layers.Conv2D(1, 4, strides=1, padding="same", kernel_initializer=kernel_initializer)(x)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_batch_begin(self, batch, logs=None):
        for i, img in enumerate(train_ol.take(self.num_img)):
            prediction_GF = self.model.gen_G(img)[0].numpy()
            prediction_GF = ((prediction_GF * 127.5) + 127.5).astype(np.uint8)
            img = ((img[0] * 127.5) + 127.5).numpy().astype(np.uint8)

            figure = np.stack((img, prediction_GF))
            figure = np.concatenate(figure, axis=1)

            cv2.imshow("olTolu", figure)

        for i, img in enumerate(train_lu.take(self.num_img)):
            prediction_FG = self.model.gen_F(img)[0].numpy()
            prediction_FG = ((prediction_FG * 127.5) + 127.5).astype(np.uint8)
            img = ((img[0] * 127.5) + 127.5).numpy().astype(np.uint8)

            figure = np.stack((img, prediction_FG))
            figure = np.concatenate(figure, axis=1)
